    ...
    for (it = keyframelist.begin(); it != keyframelist.end(); it++)
    {
        ...
        for (int i = 0; i < (int)(*it)->keypoints.size(); i++)
        {
            ...
            fprintf(keypoints_file, "%f %f %f %f %f\n", (*it)->keypoints[i].pt.x, (*it)->keypoints[i].pt.y,
                    (*it)->keypoints_norm[i].x, (*it)->keypoints_norm[i].y, (*it)->keypoints_norm[i].z);
        }
        brief_file.close();
...
void PoseGraph::loadPoseGraph()
{
                  ...
                  &keypoints_num) != EOF)
    {
        ...
        */
        ...
        vector<cv::Point3f> keypoints_norm;
        ...
        for (int i = 0; i < keypoints_num; i++)
        {
            ...
            // changed by wz loop closure
            cv::Point3f tmp_keypoint_norm;
            // double p_x, p_y, p_x_norm, p_y_norm;
            // changed by wz loop closure
            double p_x, p_y, p_x_norm, p_y_norm, p_z_norm;
            // if (!fscanf(keypoints_file, "%lf %lf %lf %lf", &p_x, &p_y, &p_x_norm, &p_y_norm))
            //     printf(" fail to load pose graph \n");
            // changed by wz loop closure
            if (!fscanf(keypoints_file, "%lf %lf %lf %lf %lf", &p_x, &p_y, &p_x_norm, &p_y_norm, &p_z_norm))
                printf(" fail to load pose graph \n");
