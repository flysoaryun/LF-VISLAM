    ...
    while (true)
    {
        ...
        if (pose_msg != NULL)
        {
            ...
            if ((T - last_t).norm() > SKIP_DIS)
            {
                ...
                vector<cv::Point3f> point_3d_normal;
                ...
                for (unsigned int i = 0; i < point_msg->points.size(); i++)
                {
                    ...
                    cv::Point2f p_2d_uv;
                    // changed by wz loop closure
                    cv::Point3f p_3d_normal;
                    double p_id;
                    // changed by wz loop closure
                    p_3d_normal.x = point_msg->channels[i].values[0];
                    p_3d_normal.y = point_msg->channels[i].values[1];
                    p_3d_normal.z = point_msg->channels[i].values[2];

                    p_2d_uv.x = point_msg->channels[i].values[3];
                    p_2d_uv.y = point_msg->channels[i].values[4];
                    p_id = point_msg->channels[i].values[5];
                    // changed by wz loop closure
                    point_3d_normal.push_back(p_3d_normal);
                ...
                // changed by wz loop closure
                // KeyFrame *keyframe = new KeyFrame(pose_msg->header.stamp.toSec(), frame_index, T, R, image,
                //                                   point_3d, point_2d_uv, point_3d_normal, point_id, sequence);
                //! changed by wz
                KeyFrame *keyframe = new KeyFrame(pose_msg->header.stamp.toSec(), frame_index, T, R, image,
                                                  point_3d, point_2d_uv, point_3d_normal, point_id, sequence, m_camera);
                m_process.lock();
                start_flag = 1;
