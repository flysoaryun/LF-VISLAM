...
public:
	...
	// 							vector<uchar> &status);
	// changed by wz loop closure
	void FundmantalMatrixRANSAC(const std::vector<cv::Point3f> &matched_3d_cur_norm,
	...
	// changed by wz loop closure
	void PnPRANSAC(const vector<cv::Point3f> &matched_3d_old_norm,
				   const std::vector<cv::Point3f> &matched_3d,
				   std::vector<uchar> &status,
				   Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old);
	...
	// changed by wz loop closure
	vector<cv::Point3f> point_3d_norm;
